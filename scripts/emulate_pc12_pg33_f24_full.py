#!/usr/bin/env python3
"""Emula OFFLINE a coleta completa de F-24 RST no caminho PG33 do PC12.
...
def emulate(exe):
    ...
    write_slot(mu, PROGRAM_RECORD + 0, '24')
    ...
    def hook(uc, address, size, _user):
        ...
        if address == PC12_ATOI:
            ...
            state['atoi'].append((text, value)); return
        if address == PC12_STRCPY:
            emulate_strcpy(uc); state['strcpy'] += 1; return
        if address == PC12_SPRINTF:
            fmt, out = emulate_sprintf(uc); state['sprintf'].append((fmt, out)); return
        if address == PROGRESS_HELPER:
            state['progress'] += 1; return_from_cdecl(uc, 0); return

    def invalid_hook(uc, access, address, size, value, _user):
        state['invalid'] = (access, address, size, value, uc.reg_read(UC_X86_REG_EIP)); return False
...
def main():
    ...
    try:
        ...
        lines = [
            'PC12 PG33 F-24 RST FULL COLLECTOR - UNICORN OFFLINE EMULATION',
            '=' * 100,
            'program=F-24 RST Y0001',
        ...
        lines = ['PC12 PG33 F-24 RST FULL COLLECTOR - UNICORN OFFLINE EMULATION', '=' * 100,
