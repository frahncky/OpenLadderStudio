...
{
    internal static class ModbusMonitorProgramV14
    {
        [STAThread]
    ...
    internal sealed class ModbusMonitorForm : Form
    {
        ...
        private ComboBox memoryAreaCombo;
        private Label memoryAreaInfo;
        ...
        private void BuildUi()
        {
            ...
            Label subtitle = NewLabel("Selecione uma área do mapa de memória para preencher automaticamente função, endereço e quantidade.", 8.2f, false, Muted);
            ...
            transportCombo = NewCombo(left, 16, y, 326);
            ...
            serialPanel.Size = new Size(354, 190);
            ...
            AddCaption(left, "MAPA DE MEMÓRIA", y); y += 24;

            memoryAreaCombo = NewCombo(left, 16, y, 218);
            memoryAreaCombo.SelectedIndexChanged += MemoryAreaChanged;

            Button reloadMap = ActionButton("RECARREGAR", 242, y, 100, PanelColor);
            reloadMap.FlatAppearance.BorderColor = Border;
            reloadMap.Click += delegate { LoadMemoryMap(true); };
            left.Controls.Add(reloadMap);
            y += 36;

            memoryAreaInfo = NewLabel("Nenhuma área selecionada.", 8.0f, false, Muted);
            memoryAreaInfo.Location = new Point(16, y);
            memoryAreaInfo.MaximumSize = new Size(326, 52);
            left.Controls.Add(memoryAreaInfo);
            y += 52;

            Button editMap = ActionButton("EDITAR MAPA DE MEMÓRIA", 16, y, 326, PanelColor);
            editMap.FlatAppearance.BorderColor = Border;
            editMap.Click += EditMemoryMap;
            left.Controls.Add(editMap);
            y += 50;
            ...
            addressBox = NewNumeric(left, 16, y + 18, 156, 0, 65535, 0);
            AddSmallLabel(left, "Quantidade", 188, y);
            quantityBox = NewNumeric(left, 188, y + 18, 154, 1, 125, 10);
            y += 64;

            Button read = ActionButton("LER DISPOSITIVO", 16, y, 204, Accent);
            ...
            statusLabel.MaximumSize = new Size(326, 90);
        ...
        private void BuildSerialPanel(Control parent)
        {
            ...
            portCombo = NewCombo(parent, 16, 22, 218);

            Button refresh = ActionButton("ATUALIZAR", 242, 22, 100, PanelColor);
        ...
        private void LoadMemoryMap(bool notify)
        {
            memoryAreas = PlcMemoryMapStore.Load(activeProfile);
            memoryAreaCombo.Items.Clear();
            memoryAreaCombo.Items.Add("Personalizado / manual");

            for (int i = 0; i < memoryAreas.Count; i++)
            {
                PlcMemoryArea a = memoryAreas[i];
                memoryAreaCombo.Items.Add(a.Name + "  •  " + AreaKindText(a.Kind) + "  •  " + a.StartAddress.ToString() + " +" + a.Length.ToString());
            }

            int match = FindMemoryAreaMatch();
            memoryAreaCombo.SelectedIndex = match >= 0 ? match + 1 : 0;
            if (notify)
            {
                statusLabel.Text = memoryAreas.Count == 0 ? "O mapa de memória deste controlador está vazio." : "Mapa de memória recarregado.";
                statusLabel.ForeColor = memoryAreas.Count == 0 ? WarningColor : Accent;
            }
        }

        private int FindMemoryAreaMatch()
        {
            if (functionCombo == null || addressBox == null || quantityBox == null) return -1;
            for (int i = 0; i < memoryAreas.Count; i++)
            {
                PlcMemoryArea a = memoryAreas[i];
                int functionIndex = FunctionIndexForArea(a.Kind);
                if (functionIndex < 0) continue;
                int maxQuantity = (functionIndex == 0 || functionIndex == 1) ? 2000 : 125;
                int expectedQuantity = Math.Max(1, Math.Min(maxQuantity, a.Length));
                if (functionCombo.SelectedIndex == functionIndex && (int)addressBox.Value == a.StartAddress && (int)quantityBox.Value == expectedQuantity)
                    return i;
            }
            return -1;
        }

        private void MemoryAreaChanged(object sender, EventArgs e)
        {
            if (memoryAreaCombo == null || memoryAreaCombo.SelectedIndex <= 0)
            {
                if (memoryAreaInfo != null)
                {
                    memoryAreaInfo.Text = "Modo manual: função, endereço e quantidade podem ser definidos livremente.";
                    memoryAreaInfo.ForeColor = Muted;
                }
                return;
            }

            int index = memoryAreaCombo.SelectedIndex - 1;
            if (index < 0 || index >= memoryAreas.Count) return;
            ApplyMemoryArea(memoryAreas[index]);
        }

        private void ApplyMemoryArea(PlcMemoryArea area)
        {
            if (area == null) return;
            int functionIndex = FunctionIndexForArea(area.Kind);
            if (functionIndex < 0)
            {
                memoryAreaInfo.Text = area.Name + ": área específica do fabricante. Selecione manualmente a função de leitura compatível.";
                memoryAreaInfo.ForeColor = WarningColor;
                statusLabel.Text = "A área selecionada não possui função Modbus genérica associada.";
                statusLabel.ForeColor = WarningColor;
                return;
            }

            functionCombo.SelectedIndex = functionIndex;
            addressBox.Value = Clamp(area.StartAddress, (int)addressBox.Minimum, (int)addressBox.Maximum);
            UpdateQuantityLimit();

            int requested = Math.Max(1, area.Length);
            int maximum = (int)quantityBox.Maximum;
            int effective = Math.Min(requested, maximum);
            quantityBox.Value = Clamp(effective, (int)quantityBox.Minimum, maximum);

            string prefix = string.IsNullOrEmpty(area.Prefix) ? "sem prefixo" : "prefixo " + area.Prefix;
            memoryAreaInfo.Text = area.Name + " • " + AreaKindText(area.Kind) + " • " + prefix + ".";
            if (requested > effective)
            {
                memoryAreaInfo.Text += " Área com " + requested.ToString() + " pontos; a leitura atual foi limitada a " + effective.ToString() + " por requisição Modbus.";
                memoryAreaInfo.ForeColor = WarningColor;
                statusLabel.Text = "Área carregada com limite de " + effective.ToString() + " pontos por requisição.";
                statusLabel.ForeColor = WarningColor;
            }
            else
            {
                memoryAreaInfo.ForeColor = Muted;
                statusLabel.Text = "Área carregada: " + area.Name + ". Função, endereço e quantidade preenchidos automaticamente.";
        ...
        private int FunctionIndexForArea(PlcMemoryAreaKind kind)
        {
            if (kind == PlcMemoryAreaKind.Coil) return 0;
            if (kind == PlcMemoryAreaKind.DiscreteInput) return 1;
            if (kind == PlcMemoryAreaKind.HoldingRegister) return 2;
            if (kind == PlcMemoryAreaKind.InputRegister) return 3;
            return -1;
        }
        ...
        private PlcMemoryArea SelectedMemoryArea()
        {
            if (memoryAreaCombo == null || memoryAreaCombo.SelectedIndex <= 0) return null;
            int index = memoryAreaCombo.SelectedIndex - 1;
            return index >= 0 && index < memoryAreas.Count ? memoryAreas[index] : null;
        }

        private string FormatDisplayAddress(int address)
        {
            PlcMemoryArea area = SelectedMemoryArea();
            if (area == null || string.IsNullOrEmpty(area.Prefix)) return address.ToString();
            int width = Math.Max(4, address.ToString().Length);
            return area.Prefix + address.ToString("D" + width.ToString());
        }
            ...
            try
            {
                ...
                if (result.Bits.Length > 0)
                {
                    for (int i = 0; i < result.Bits.Length; i++)
                        resultGrid.Rows.Add(i.ToString(), FormatDisplayAddress(address + i), result.Bits[i] ? "ON" : "OFF", result.Bits[i] ? "1" : "0");
                }
                else
                {
                ...
                statusLabel.Text = "Leitura concluída: " + quantity.ToString() + " ponto(s)" + areaText + ". Perfil salvo automaticamente.";
                statusLabel.ForeColor = Accent;
            }
            catch (Exception ex)
            {
