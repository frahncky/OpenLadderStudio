...
namespace ModernPC12
{
    ...
    internal static class StudioLayoutV68
    {
        ...
        private static readonly Color PanelColor = Color.FromArgb(252, 253, 255);
        ...
        private static void RemoveLegacyRails(UniversalStudioForm form)
        {
        ...
        private static void RebuildMenu(UniversalStudioForm form)
        {
            ...
            projeto.DropDownItems.Add(Item("Compilar / validar", delegate { InvokeLadder(form, "ValidateProject", new object[] { true }); }));
            ...
            janela.DropDownItems.Add(Item("Programa principal", delegate { SafeCall(form, "ShowLadder", null); HideEmbeddedLadderToolbox(form); }));

            ToolStripMenuItem ajuda = Menu("Ajuda");
            ajuda.DropDownItems.Add(Item("Sobre o OpenLadder Studio", delegate
            {
                MessageBox.Show(form,
                    "OpenLadder Studio v0.68\r\n\r\nAmbiente de engenharia Ladder com interface industrial clara e arquitetura multi-PLC.",
        ...
        private static void RebuildWorkspace(UniversalStudioForm form)
        {
            ...
            Panel right = BuildInstructionRail(form);
            Panel left = BuildProjectRail(form);
        ...
        private static Panel BuildProjectRail(UniversalStudioForm form)
        {
            ...
            rail.Width = 304;
            rail.BackColor = PanelColor;

            V68HeaderPanel header = new V68HeaderPanel();
            header.Dock = DockStyle.Top;
            header.Height = 38;
            header.Title = "Projeto";
            header.Icon = StudioIcon.Folder;

            Panel properties = new Panel();
            properties.Dock = DockStyle.Bottom;
            properties.Height = 230;
            properties.BackColor = Color.White;
            properties.Padding = new Padding(12, 8, 12, 8);
            ...
            tree.ImageList = BuildTreeImages();
            ...
            programs.Expand();
            vars.Expand();
            tree.SelectedNode = programs.Nodes[0];
        ...
        private static Panel BuildInstructionRail(UniversalStudioForm form)
        {
            ...
            header.Dock = DockStyle.Top;
            header.Height = 38;
            header.Title = "Caixa de Instruções";
            header.Icon = StudioIcon.Ladder;

            Panel searchWrap = new Panel();
            searchWrap.Dock = DockStyle.Top;
            searchWrap.Height = 48;
            searchWrap.BackColor = PanelColor;
            ...
            instructionFlow.BackColor = PanelColor;
        ...
        private static void PopulateInstructions(UniversalStudioForm form, string filter)
        {
            ...
            for (int i = 0; i < specs.Count; i++)
            {
                ...
                if (needle.Length > 0 &&
                    spec.Text.ToLowerInvariant().IndexOf(needle) < 0 &&
                    spec.Category.ToLowerInvariant().IndexOf(needle) < 0)
                    continue;
        ...
        private static void RestyleConsole(UniversalStudioForm form)
        {
            ...
            foreach (Control c in wrap.Controls)
            {
                ...
                if (head == null || head.Dock != DockStyle.Top) continue;
        ...
        {
            MessageBox.Show(form,
                "A interface de transferência está pronta, mas o download de programa para PLC físico permanece bloqueado enquanto o driver do controlador não declarar essa capacidade de forma validada.\r\n\r\nUse o Simulador para executar o Ladder localmente.",
        ...
        private static void RefreshProperties(UniversalStudioForm form)
        {
            if (propertyProject != null)
            {
                ...
                string value = project == null ? string.Empty : (project.Text ?? string.Empty).Trim();
                propertyProject.Text = string.IsNullOrEmpty(value) || value == "Sem nome" ? "Main" : value;
            }

            if (propertyController != null)
            {
                Label device = Field<Label>(form, "deviceValue");
                string value = device == null ? string.Empty : (device.Text ?? string.Empty).Trim();
                propertyController.Text = string.IsNullOrEmpty(value) ? "-" : value;
            }
        }
        ...
        {
            V68ToolbarButton button = new V68ToolbarButton();
            button.Text = text;
            button.Icon = icon;
            button.IconColor = color;
            button.Location = new Point(x, 5);
            button.Size = new Size(74, 73);
    ...
    internal sealed class V68ToolbarButton : Control
    {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            Color back = pressed ? Color.FromArgb(221, 235, 249) :
                hover && Enabled ? Color.FromArgb(237, 245, 253) : Color.White;
            ...
            TextRenderer.DrawText(g, Text, Font, new Rectangle(2, 47, Width - 4, 20),
                Enabled ? Color.FromArgb(31, 43, 56) : Color.FromArgb(150, 160, 170),
                TextFormatFlags.HorizontalCenter | TextFormatFlags.VerticalCenter | TextFormatFlags.EndEllipsis);

            if (hover && Enabled)
                using (SolidBrush b = new SolidBrush(semantic))
                    g.FillRectangle(b, 10, Height - 3, Math.Max(1, Width - 20), 2);
    ...
    internal sealed class V68BrandPanel : Control
    {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            using (LinearGradientBrush brush = new LinearGradientBrush(ClientRectangle,
                Color.FromArgb(24, 82, 145), Color.FromArgb(15, 63, 116), 0f))
            {
                Point[] poly = new Point[] {
                    new Point(28, 0), new Point(Width, 0), new Point(Width, Height),
                    new Point(0, Height), new Point(20, 58)
                };
                g.FillPolygon(brush, poly);
            }

            StudioGlyph.Draw(g, StudioIcon.Ladder, new Rectangle(32, 18, 34, 34), Color.White);
            TextRenderer.DrawText(g, "OpenLadder Studio",
                new Font("Segoe UI Semibold", 14.0f, FontStyle.Bold), new Point(78, 15), Color.White);
            TextRenderer.DrawText(g, "PROGRAME IDEIAS. CONTROLE O AMANHÃ.",
                new Font("Segoe UI", 7.1f), new Point(80, 47), Color.FromArgb(220, 234, 248));
        }
    }

    internal sealed class V68HeaderPanel : Panel
    {
        public StudioIcon Icon = StudioIcon.None;
        public string Title = string.Empty;

        public V68HeaderPanel()
        {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            using (Pen p = new Pen(Color.FromArgb(207, 216, 226)))
            ...
            TextRenderer.DrawText(e.Graphics, Title,
                ...
                new Rectangle(38, 0, Math.Max(1, Width - 48), Height), Color.FromArgb(31, 43, 56),
    ...
    internal sealed class V68SectionLabel : Control
    {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            using (SolidBrush b = new SolidBrush(Color.FromArgb(241, 246, 251)))
                e.Graphics.FillRectangle(b, ClientRectangle);
            using (Pen p = new Pen(Color.FromArgb(220, 227, 235)))
                e.Graphics.DrawLine(p, 0, Height - 1, Width, Height - 1);

            TextRenderer.DrawText(e.Graphics, "›",
                new Font("Segoe UI Semibold", 11f, FontStyle.Bold), new Rectangle(8, 0, 20, Height),
                Color.FromArgb(28, 112, 220), TextFormatFlags.VerticalCenter);
            TextRenderer.DrawText(e.Graphics, Text, Font,
                new Rectangle(29, 0, Math.Max(1, Width - 36), Height), Color.FromArgb(31, 43, 56),
                TextFormatFlags.VerticalCenter | TextFormatFlags.EndEllipsis);
        }
    }
    ...
    internal sealed class V68InstructionButton : Control
    {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            TextRenderer.DrawText(e.Graphics, Text, Font,
                new Rectangle(39, 0, Math.Max(1, Width - 45), Height),
                Enabled ? Color.FromArgb(31, 43, 56) : Color.FromArgb(145, 154, 164),
                TextFormatFlags.VerticalCenter | TextFormatFlags.EndEllipsis);

            using (Pen p = new Pen(Color.FromArgb(235, 239, 244)))
                e.Graphics.DrawLine(p, 0, Height - 1, Width, Height - 1);

            if (Focused && Enabled)
                ControlPaint.DrawFocusRectangle(e.Graphics, new Rectangle(4, 3, Width - 8, Height - 6));
        }
    }
